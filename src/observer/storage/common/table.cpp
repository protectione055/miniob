...
RC Table::open(const char *meta_file, const char *base_dir, CLogManager *clog_manager)
{
  ...
  for (int i = 0; i < index_num; i++) {
    ...
    const char **index_fields = index_meta->fields();
    const int index_field_num = index_meta->num_fields();
    std::vector<const FieldMeta *> field_metas;
    for(int i = 0; i < index_field_num; i++) {
      const FieldMeta *field_meta = table_meta_.field(index_fields[i]);
...
RC Table::create_index(Trx *trx, const char *index_name, const char **attribute_names, int num_attributes, bool unique)
{
  ...
  if (table_meta_.index(index_name) != nullptr) {
    LOG_INFO("Invalid input arguments, table name is %s, index %s exists",
             name(), index_name);
