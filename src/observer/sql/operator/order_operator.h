...
public:
  ...
  static void set_order_fields(std::vector<std::tuple<FieldExpr, int>> order_fields){
    order_fields_.swap(order_fields);
  }
public:
  static std::vector<std::tuple<FieldExpr, int>> order_fields_;
private:
  // bool test(Tuple *tuple);
  static bool order_compare(const Tuple *a, const Tuple *b);
private:
