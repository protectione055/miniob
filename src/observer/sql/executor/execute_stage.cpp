...
void tuple_to_string(std::ostream& os, const Tuple& tuple) {
    ...
    for (int i = 0; i < tuple.cell_num(); i++) {
        ...
        if (rc != RC::SUCCESS) {
            LOG_WARN("failed to fetch field of cell. index=%d, rc=%s", i,
                     strrc(rc));
            break;
        }
...
RC ExecuteStage::do_select(SQLStageEvent* sql_event) {
    ...
    ;
    if (select_stmt->do_aggragate()) {
        //插入聚合Executor
        AggregateOperator aggre_oper(select_stmt->query_fields());
...
RC ExecuteStage::do_create_table(SQLStageEvent* sql_event) {
    ...
    RC rc =
        db->create_table(create_table.relation_name,
...
RC ExecuteStage::do_insert(SQLStageEvent* sql_event) {
    ...
    RC rc = table->insert_record(trx, insert_stmt->value_amount(),
                                 insert_stmt->values());
    if (rc == RC::SUCCESS) {
        if (!session->is_trx_multi_operation_mode()) {
            CLogRecord* clog_record = nullptr;
            rc = clog_manager->clog_gen_record(
                CLogType::REDO_MTR_COMMIT, trx->get_current_id(), clog_record);
...
RC ExecuteStage::do_update(SQLStageEvent* sql_event) {
    ...
    Operator* scan_oper =
        try_to_create_index_scan_operator(update_stmt->filter_stmt());
    if (nullptr == scan_oper) {
